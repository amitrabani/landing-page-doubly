...
type Props = {
  /** Optional headline override. Defaults to the localized "Try it in the app". */
  headline?: string;
  /** One short paragraph explaining why the app is more than the web tool. */
  body: string;
  /** Optional button label override. Defaults to the localized "Try it in the app". */
  ctaLabel?: string;
};

export default function SoftAppCTA({ headline, body, ctaLabel }: Props) {
  const strings = useT().toolWidgets.chrome;
  const title = headline ?? strings.tryInApp;
  const label = ctaLabel ?? strings.tryInApp;
      ...
      <div className="flex flex-col items-start gap-6 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-col items-start gap-3 sm:items-end sm:shrink-0">
          <a
            ...
            aria-label={strings.appStoreAria(label)}
